...
func calculateJacobian(params []float64, points []Point) [][]float64 {
	...
	for i, p := range points {
		...
		// 对 theta 的偏导数（严格推导）
		jac[i][0] = -(x*(1+math.Pow(math.Tan(theta), 2)) -
...
func solveNewton(params []float64, points []Point, maxIter int, tol float64) ([]float64, error) {
	for iter := 0; iter < maxIter; iter++ {
		...
		// 添加动态正则化项
		lambda := 1e-6 * math.Max(math.Abs(JTJ[0][0]), math.Max(math.Abs(JTJ[1][1]), math.Abs(JTJ[2][2])))
		for i := 0; i < 3; i++ {
			JTJ[i][i] += lambda
		}
...
func main() {
	...
	initialParams := []float64{
		math.Atan(1.5), // 初始角度arctan(1.5) ≈ 56.31度
		4.0,            // 初始速度
		9.8,            // 重力加速度
	}
